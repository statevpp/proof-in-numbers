...
"Proof in Numbers" is an automated YouTube Shorts channel. This project (a set of scripts running on GitHub Actions) does not collect, store, or process any personal data from viewers or from any third party.
...
- - We use the YouTube Data API v3 solely to upload our own videos to our own YouTube channel (Proof in Numbers) and to optionally set a custom thumbnail for those videos.
  - - We do not read, collect, or store any data belonging to YouTube viewers or other channels.
    ...
    - - The only credentials used are our own OAuth tokens (client ID/secret and refresh token), stored as encrypted GitHub Actions secrets, used exclusively to authenticate our own channel's video uploads.
